...
class MoveGroupInteface(object):
	...
	def get_current_pose(self):
		return self.move_group_commander.get_current_pose().pose
	...
	def capture_image(self):
		...
		def image_callback(msg):
			...
			try:
				...
				cv2.imwrite('/home/yan/COWP_img/'+str(time)+'.jpeg', cv2_img)
				rospy.sleep(1)

		IM_counter = 0
		while not rospy.is_shutdown() and IM_counter <=2:
			try:
				image_topic = "/usb_cam/image_raw"
				rospy.Subscriber(image_topic, Image, image_callback)
				IM_counter = IM_counter + 1
				rospy.sleep(0.1)
				print('IM_counter:{}'.format(IM_counter))
			except:
				pass
# -----------------------------------------
# initialize
...
wpose = demo.get_current_pose()
x_init = wpose.position.x
y_init = wpose.position.y
z_init = wpose.position.z

# -----------------------------------------
# take image 1
# -----------------------------------------
cartesian_plan, fraction = demo.plan_cartesian_path(x_init, y_init, z_init)
...
# take image 2
# -----------------------------------------
cartesian_plan, fraction = demo.plan_cartesian_path(x_init + 0.2, y_init, z_init)
demo.execute_plan(cartesian_plan)
demo.capture_image()
print('-'*30)
...
# take image 3
# -----------------------------------------
cartesian_plan, fraction = demo.plan_cartesian_path(x_init - 0.2, y_init  + 0.2, z_init)
demo.execute_plan(cartesian_plan)
demo.capture_image()
print('-'*30)
...
# take image 4
# -----------------------------------------
cartesian_plan, fraction = demo.plan_cartesian_path(x_init, y_init - 0.2, z_init + 0.2)
demo.execute_plan(cartesian_plan)
demo.capture_image()
print('-'*30)
...
# take image 5
# -----------------------------------------
cartesian_plan, fraction = demo.plan_cartesian_path(x_init, y_init, z_init - 0.2)
demo.execute_plan(cartesian_plan)
demo.capture_image()
print('-'*30)